...
The usage grew exponentially over the coming months, and the kinds of usage are becoming increasingly varied: (starting flows based on [file detection from buckets](/plugins/plugin-gcp/gcs/io.kestra.plugin.gcp.gcs.trigger), syncing data warehouses directly from operation [postgres database](/plugins/plugin-jdbc-postgres/l/io.kestra.plugin.jdbc.postgresql.copyout), developing a simple python program to [fetch APIs from a partner](/plugins/core/tasks/scripts/io.kestra.core.tasks.scripts.python), starting long-running data science processes [over Kubernetes](/plugins/plugin-kubernetes/io.kestra.plugin.kubernetes.podcreate), [fetching data from Google Drive](/plugins/plugin-googleworkspace/sheets/io.kestra.plugin.googleworkspace.sheets.read) and writing results back to [Google Sheets](/plugins/plugin-googleworkspace/drive/io.kestra.plugin.googleworkspace.drive.create), and so on.).
